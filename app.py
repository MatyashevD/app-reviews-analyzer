...
def display_search_results(results: dict):
    ...
    with container:
        ...
        for app in all_results:
            if selected_count >= 2 and not (
                (st.session_state.selected_gp_app and app['id'] == st.session_state.selected_gp_app['id']) or 
...
def main():
    ...
    st.title("📱 Сравнительный анализ мобильных приложений")
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[{
                "role": "system",
                "content": """Сгенерируй анализ отзывов в формате:
                1. Основные проблемы (3-6 пунктов)
                2. Распределение тональности (проценты)
                3. Рекомендации по улучшению
